...
# ====== CONFIG ======
DATA_ROOT = "/Volumes/Extreme SSD/cataract-101"  # <-- CHANGE if needed

CLIP_LEN = 16           # frames per clip
FRAME_SUBSAMPLE = 15    # gap between frames (in frames)
BATCH_SIZE = 2
EPOCHS = 7              # slightly longer, still baseline
LR = 1e-4
VAL_RATIO = 0.2
RANDOM_SEED = 42
NUM_EVAL_CLIPS = 8      # clips per video at eval time (video-level metrics)
...
def eval_epoch_clip_level(model, loader, criterion):
    """
    Simple clip-level validation (for quick feedback during training).
    """
    model.eval()
    running_loss = 0.0
    correct = 0
...
def main():
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,   # simple shuffle, we handle imbalance with class weights
    ...
    # ====== CLASS-WEIGHTED LOSS (based on train set) ======
    ...
    for epoch in range(EPOCHS):
        ...
        # Save best model so far (clip-level val acc as proxy)
...
def evaluate_auc_acc():
    """
    Proper video-level evaluation:
    - For each validation video, sample NUM_EVAL_CLIPS random clips
    - Average probabilities across clips
    - Compute AUC & Accuracy at video level
    """
    meta = build_meta(DATA_ROOT)
    train_ids, val_ids = stratified_split(meta, val_ratio=VAL_RATIO, seed=RANDOM_SEED)
    ...
    for idx in range(len(val_ds)):
        ...
        for _ in range(NUM_EVAL_CLIPS):
            clip, _ = val_ds[idx]  # each call samples a different random clip
            clip = clip.unsqueeze(0).to(device)  # [1, T, C, H, W]
            logits = model(clip)
            prob = torch.softmax(logits, dim=1)[:, 1]  # prob of class 1
            clip_probs.append(prob.item())

        mean_prob = float(np.mean(clip_probs))
        all_labels.append(label)
        all_scores.append(mean_prob)
